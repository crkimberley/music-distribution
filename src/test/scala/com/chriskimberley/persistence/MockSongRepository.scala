...
import com.chriskimberley.domain.ReleaseStatus.Approved
import com.chriskimberley.domain.error.MusicServiceError
import com.chriskimberley.persistence.MockSongRepository.approvedReleaseDateOfToday

case class MockSongRepository(ref: Ref[Seq[Song]]) extends SongRepository {
  override def save(song: SongIn): IO[MusicServiceError, Song] =
    for {
      uuid <- ZIO.succeed(randomUUID())
      _ <- ref.update(_ :+ Song(SongId(uuid), song.title, approvedReleaseDateOfToday))
    } yield Song(SongId(uuid), song.title)
...
object MockSongRepository {
  val layer: ULayer[SongRepository] =
    ZLayer(Ref.make(Seq.empty[Song]).map(MockSongRepository(_)))
  ...
  val prefilledLayer: ULayer[SongRepository] = ZLayer {
    Ref
      .make(
        Seq(
          Song(songIds(0), "songA", approvedReleaseDateOfToday),
          Song(songIds(1), "songB", approvedReleaseDateOfToday)
        )
      )
      .map(MockSongRepository(_))
  }
}
